...
int main(int argc,char **args) {
	...
	int mxn=atoi(args[1]),diffa=atoi(args[2]);
	cerr<<"n"<<mxn<<"diffa"<<diffa<<endl;
	ll t=5;
	printf("%lld\n",t);
	while(t--){
		ll n=randint(max(1,mxn-10), mxn);
	printf("%lld\n",n);
		if(diffa){
			for(int i=1;i<=n;i++)
				arr[i]=i;
		...
		else shuffle(arr+1, arr+1+n, myrand);
		for(int i=1;i<=n;i++)
			brr[i]={arr[i],i};
		shuffle(brr+1, brr+1+n, myrand);
		for(int i=1;i<=n;i++)
			printf("%lld %lld\n",brr[i].fi,brr[i].se);
