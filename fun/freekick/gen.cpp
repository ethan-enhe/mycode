...
const ll INF = 1e18;
const ll MXN = 1e6 + 5;
double ratel = 1, rater = 1;
double side = 0.00005;
void gen(ll mxn, ll mxm, ll mxw) {
    ll w = ri(max(1ll, mxw - 10), mxw);
    ll m = ri(max(1ll, mxm - 100), mxm);
    ll tmp = ri(max(1ll, ll(ratel * m)), max(1ll, ll(rater * m)));
    vector<vector<pi>> lim;
    vector<ll> x;
    lim.resize(tmp);
    x.resize(tmp);
    for (ll i = 1; i < x.size(); i++) x[i] = x[i - 1] + ri(1, mxn / tmp);
    cout << mxn << " " << m << " " << w << nl;
    while (m--) {
        ll ind = ri(0, tmp - 1), l, r;
        do {
            l = ri(-side * w, w);
            r = ri(0, w * (1. + side));
            if (l > r) swap(l, r);
        ...
        lim[ind].push_back({l, r});
    }
    for (ll i = 0; i < x.size(); i++)
        for (auto [l, r] : lim[i]) cout << x[i] << " " << l << " " << r << nl;
...
int main(int argc, char **args) {
    ...
    cin.tie(0);
    ll t = 10, mxn = atoll(args[1]), mxm = atoll(args[2]), mxw = atoll(args[3]);
    sscanf(args[4],"%lf", &ratel);
    sscanf(args[5],"%lf", &rater);
    sscanf(args[6],"%lf", &side);
    cout << t << nl;
    while (t--) gen(mxn, t <= 2 ? mxm / 2 : mxm, mxw);
