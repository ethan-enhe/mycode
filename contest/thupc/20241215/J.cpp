...
bool color[MXN];
vector<ll> G[MXN];
ll dp[MXN][2][3]; // 子树，子树内被什么颜色主导，及其深度
void dfs(ll p, ll fa) {
    if (color[p]) {
        dp[p][1][0] = 0;
    } else {
        ...
        if (color[p]) {
            for (ll i = 0; i < 2; i++) {
                for (ll j = 0; j < 3; j++) {
                    // 根被涂色，或者白色主导，并且深度<=1
                    if (j == 0 || (i == 0 && j <= 1)) {
                        umn(nxt[1][0], dp[p][1][0] + dp[nx][i][j]);
                    }
                }
            }
        } else {
            for (ll i = 0; i < 2; i++) {
                for (ll j = 0; j < 3; j++) {
                    // 涂成白色，只要求对面白色主导，或者黑色主导，但是深度<=1
                    if (i == 0 || (i == 1 && j <= 1)) umn(nxt[0][0], dp[p][0][0] + dp[nx][i][j]);
                    // 不涂色，要求对面白色主导，或者黑色主导，深度=0
                    if (i == 0 || (i == 1 && j == 0)) {
                        for (int ci = 0; ci < 2; ci++)
                            for (int cj = 0; cj < 3; cj++) {
                                ll domd, domc;
                                tie(domd, domc) = min(pi{cj, ci}, pi{min(2ll, j + 1), i});
                                umn(nxt[domc][domd], dp[p][ci][cj] + dp[nx][i][j]);
                            }
                    }
                }
            }
        }
        memcpy(dp[p], nxt, sizeof(nxt));
    }
}
int main() {
