...
int main() {
    ...
    sort(brr + 1, brr + 1 + n, [](pi x, pi y) { return x.fi < y.fi; });
    for (ll i = 1; i <= n + 1; i++) sum[i] = sum[i - 1] + brr[i].fi;
    ll ind = 1, cur = 0,  ans = 0;
    for (ll i = n; i; i--) {
        if (arr[i].fi > cur) {
            ll csum = sum[ind - 1] + cur * (n + 1 - ind);
            ll nxcur = arr[i].fi;
            while (ind <= n && brr[ind].fi <= nxcur) ++ind;
            ll nxsum = sum[ind - 1] + nxcur * (n + 1 - ind);
            ans += (nxsum - csum) * i;
            cur=nxcur;
        }
    }
