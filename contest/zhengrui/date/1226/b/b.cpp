...
void dfs(int p){
	if(p==n+1){
		ll mn=INF;
		for(int i=0;i<=lim && i<=ind[1];i++)
			for(int j=0;i+j<=lim && j<=ind[2];j++){
				ll k=lim-i-j,cnt=0,mx=0,sum=0;
				if(k>ind[3])continue;
				for(int l=1;l<=i;l++)ord[++cnt]={tmp[1][l],1};
				for(int l=1;l<=j;l++)ord[++cnt]={tmp[2][l],2};
				for(int l=1;l<=k;l++)ord[++cnt]={tmp[3][l],3};
				sort(ord+1,ord+1+lim);
				for(int l=lim;l;l--){
					sum+=ord[l].se;
					umx(mx,sum+ord[l].fi);
				}
