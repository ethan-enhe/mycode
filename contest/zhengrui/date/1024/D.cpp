...
const ll MXQ=1e5+5,MXN=35;
ll n,m,ql=MXQ,qr=MXQ-1;
ll arr[MXQ<<1][MXN],va[MXQ<<1][MXN];
...
inline void cal(){
	...
	for(int i=ql+1;i<=qr;i++)
		for(int j=1;j<=n;j++)
			va[i][j]=(va[i-1][j]+va[i-1][j-1]+va[i-1][j+1])*arr[i][j]%P;
	ll res=0;
	for(int i=1;i<=n;i++)
		res=(res+va[qr][i])%P;
	printf("%lld\n",res);
}
il void read(ll ind){for(int i=1;i<=n;i++)scanf("%lld",arr[ind]+i);}
il void solve(){
	scanf("%lld%lld",&m,&n);
	while(m--){
		ll opt;
