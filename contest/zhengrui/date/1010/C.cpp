...
const ll MXN=15,MXV=805;
ll n,m,FULL;
ll arr[MXN],brr[MXN],cost[MXN][MXN],cnt[MXV],va[MXV];
ll dp[1<<MXN][MXN+5][2];

il ll cal(ll x){
	//cout<<"CAL"<<x<<endl;
	for(ll i=1;i!=FULL;i++){
		ll ai=__builtin_popcount(i)-1;
		for(ll j=0;j<=m;j++){
			dp[i][j][0]=dp[i][j][1]=0;
			for(ll k=0;k<n;k++)
				if((i>>k)&1){
					if(cost[ai][k]<x){
						madd(dp[i][j][0],dp[i^(1<<k)][j][0]);
						madd(dp[i][j][1],dp[i^(1<<k)][j][1]);
					}
					else if(j){
						if(cost[ai][k]==x)
							madd(dp[i][j][1],dp[i^(1<<k)][j-1][0]);
						else{
							madd(dp[i][j][0],dp[i^(1<<k)][j-1][0]);
							madd(dp[i][j][1],dp[i^(1<<k)][j-1][1]);
						}
					}
				}
...
il void solve(){
	...
	for(int i=0;i<n;i++)
		for(int j=0;j<n;j++){
			cost[i][j]=cnt[arr[i]+brr[j]]--;
			va[cost[i][j]]=arr[i]+brr[j];
		}
	dp[0][0][0]=1;
	ll res=0;
	for(int i=1;i<=n*n;i++)
