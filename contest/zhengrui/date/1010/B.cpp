...
il void solve(){
	...
	for(int i=1;i<=n;i++){
		...
		if(last[arr[i]]){
			ll tmp=i-last[arr[i]];
			if(!lastd[arr[i]]){
				if(last[arr[i]]>tmp)f=0;
				lastd[arr[i]]=tmp;
			}
			else if(lastd[arr[i]]!=tmp)f=0;
		}
		last[arr[i]]=i;
	}
	for(int i=1;i<=n;i++){
		//cerr<<lastd[arr[i]]<<endl;
		if(lastd[arr[i]] && n-last[arr[i]]>=lastd[arr[i]])
			f=0;
		last[arr[i]]=lastd[arr[i]]=0;
	}
	cout<<(f?"Yes":"No")<<nl;
