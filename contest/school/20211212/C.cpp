...
//}}}
const ll INF = 1e18, P = 1e9 + 7, MXN = 5e6 + 5;
...
ll s[MXN],s1[MXN],dp[MXN];


ll nodec;
pi v[MXN];
ll cal(pi f,ll x){return x*f.fi+f.se;}
#define ls p<<1
#define rs p<<1|1
void upd(ll p,ll l,ll r,pi f){
	ll mid=(l+r)>>1;
	if(cal(f,mid)>cal(v[p],mid))swap(f,v[p]);
	if(l==r)return;
	f.fi>v[p].fi?upd(rs,mid+1,r,f):upd(ls,l,mid,f);
}
ll que(ll p,ll l,ll r,ll qi){
	if(l==r)return cal(v[p],qi);
	ll mid=(l+r)>>1;
	return max(cal(v[p],qi),qi>mid?que(rs,mid+1,r,qi):que(ls,l,mid,qi));
}
void ins(ll ind){
	upd(1,1,n,{-s[ind],dp[ind]+s1[ind]});
}
