...
const char nl = '\n';
const ll INF = 1e18;
const ll MXN = 2005, MXM = 3e5 + 5;

ll n, m;
pi c[MXN];
pi dis[MXN * MXN];
using bs = bitset<MXN>;
// greater,less than current dis
bs g[MXN], l[MXN];
bool isc[MXM];
int main() {
    ...
    for (ll i = 1; i < MXM; i++)
        if (!isc[i])
            for (ll j = i << 1; j < MXM; j += i) isc[j] = 1;
    int t;
    cin >> t;
    while (t--) {
        cin >> n >> m;
        ...
        for (ll i = 1; i <= n; i++) {
            cin >> c[i];
            for (ll j = 1; j < i; j++) dis[++m] = {i, j};
            g[i].set();
            l[i].reset();
        }
        auto _dis = [](pi x) -> ll { return abs(x.fi) + abs(x.se); };
        auto cmp = [&](pi &x, pi &y) -> bool { return _dis(c[x.fi] - c[x.se]) < _dis(c[y.fi] - c[y.se]); };
        sort(dis + 1, dis + 1 + m, cmp);
        ll ans = 0;
        for (ll i = 1; i <= m; i++) {
            ll u = dis[i].fi, v = dis[i].se;
            g[u][v] = g[v][u] = 0;
            if (!isc[_dis(c[u] - c[v])]) ans += (g[v] & l[u]).count() + (l[v] & g[u]).count();
            l[u][v] = l[v][u] = 1;
        }
        cout << ans << nl;
    }
