...
void dfs(int p) {
    ll curmod = p * 2 % m;
    q[curmod][++top[curmod]] = p;

    ll lastmod = p % m;
    ll id = lower_bound(q[lastmod].begin() + 1, q[lastmod].begin() + 1 + top[lastmod], p / 2 + 1) - q[lastmod].begin();
    ans = ans * (top[lastmod] - id + 2) % P;

    for (ll nx : g[p]) dfs(nx);
    --top[curmod];
}
...
int main() {
    ...
    while (t--) {
        ...
        for (ll i = 2; i <= n; i++) pre[i] = find(pre[i - 1], str[i]);
        for (ll i = 0; i <= n; i++) {
            g[i].clear();
            q[i].resize(1);
            top[i] = 0;
