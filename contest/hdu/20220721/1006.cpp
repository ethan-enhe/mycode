...
db solve(db f0) {
    ...
    for (ll i = k - 1; ~i; i--) {
        ...
        for (ll j = 0; j < LG; j++) {
            ...
            f[i] = 0;
            db invAB = 1. / A / B;
            for (ll a = 0; a < A; a++) { //一次函数与x取min求和
                db b = f[i + 1] * a / A + x * (A - a) / A;
                db k = (f0 * (A - a) - x * (A - a)) * invAB;
                db itc = (x - b) / k; //交点
                if (k > 0) {
                    ll ind = floor(itc);
                    ind = max(-1ll, min(ind, B - 1));
                    f[i] += (b * 2 + ind * k) * (ind + 1) / 2 + x * (B - ind - 1);
                } else {
                    ll ind = ceil(itc);
                    ind = min(B, max(ind, 0ll));
                    f[i] += (b * 2 + (ind + B - 1) * k) * (B - ind) / 2 + x * ind;
                }
            }
            f[i] = f[i] * invAB + 1;
            if (f[i] > x)
