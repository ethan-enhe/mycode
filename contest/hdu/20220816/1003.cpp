...
int main() {
    ...
    while (t--) {
        ...
        stk[top = 0] = n + 1;
        for (ll i = n; i; i--) {
            while (arr[i] > arr[stk[top]]) --top;
            stk[top + 1] = i;
            cnt[top + 1] = cnt[top] + (stk[top] != i + 1);
            ++top;
            for (auto q : g[i]) {
                //找到最后大于的位置
                ll l = 0, r = top;
                ...
                ll res = cnt[top] - cnt[l];
                /* if (i == 7) cout << stk[l+1] << '!' << endl; */
                if (stk[l] != stk[l + 1] + 1 && stk[l + 1] == q.fi) --res;
                ans[q.se] = res;
            }
        }
