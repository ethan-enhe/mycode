...
ll n, m, r, arr[MXN], pre[MXN];
mod dp[MXN][MXN];
void cal() {
    for (ll i = 0; i <= n; i++)
        for (ll j = 0; j <= m; j++) dp[i][j] = 0;
    dp[0][0] = 1;
    for (ll i = 0, k = 1; i < n; i++) {
        for (ll j = 0; j <= m; j++)
            if (dp[i][j].v) {
                dp[i + 1][j + 1] += dp[i][j];
                if (j - min(k, pre[i + 1] - 1) > 0) dp[i + 1][j] += dp[i][j] * (j - min(k, pre[i + 1] - 1));
            }
        k = min(pre[i + 1], k + 1);
    }
    cout << dp[n][m] << nl;
...
int main() {
    ...
    while (t--) {
        ...
        for (ll i = 1; i <= n; i++) {
            pre[i] = pre[i - 1] + 1;
            while (arr[i - pre[i] + 1] + r <= arr[i]) --pre[i];
        }
        cal();
        // pre表示最靠近i的多少个无法跳到i
        /* cout << cal(3) << " " << cal(2) << " " << cal(1); */
        /* cout << ans << nl; */
    }
    return 0;
}
