...
int main() {
    ...
    for (ll i = 2; i < MXN; i++)
        if (!isc[i]) {
            pri.push_back(i);
            for (ll j = i << 1; j < MXN; j += i) isc[j] = 1;
        }
    cin >> n >> m;
    while (m--) {
        ll x, y;
        ...
        if (__gcd(x, y) == 1)
            ...
            fx = fac(x), fy = fac(y);
            fx.insert(fx.end(), all(fy));
            sort(all(fx), [](ll x, ll y) { return x > y; });
            unq(fx);
            ans = 0;
            dfs(1, 0, 0);
            cout << "2 " << ans + (__gcd(x, y) == 2) << nl;
        }
    }
    return 0;
}
