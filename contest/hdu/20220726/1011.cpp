...
int main() {
    ...
    while (t--) {
        cin >> n >> m;
        for (ll i = 1; i <= n; i++) cin >> arr[i].x >> arr[i].y >> arr[i].w;
        sort(arr + 1, arr + 1 + n, [](p a, p b) { return a.w > b.w; });
        mx[0][0] = mx[0][1] = mx[0][2] = mx[0][3] = -INF;
        for (ll i = 1; i <= n; i++) {
            mx[i][0] = max(mx[i - 1][0], arr[i].x + arr[i].y);
            mx[i][1] = max(mx[i - 1][1], arr[i].x - arr[i].y);
            mx[i][2] = max(mx[i - 1][2], -arr[i].x + arr[i].y);
            mx[i][3] = max(mx[i - 1][3], -arr[i].x - arr[i].y);
        }
        auto mxdis = [](ll x, ll y, ll i) {
            return max({mx[i][0] - x - y, mx[i][1] - x + y, mx[i][2] + x - y, mx[i][3] + x + y});
        };
        while (m--) {
            ll x, y;
            cin >> x >> y;
            ...
            while (l < r) {
                ll mid = (l + r + 1) >> 1;
                if (mxdis(x, y, mid) <= arr[mid].w)
                    l = mid;
                else
                    r = mid - 1;
            }
            ll ans = min(mxdis(x, y, l), arr[l].w);
            if (l != n) umx(ans, min(mxdis(x, y, l + 1), arr[l + 1].w));
            cout << ans << nl;
        }
    }
    return 0;
