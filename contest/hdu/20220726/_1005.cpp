...
const char nl = '\n';
const ll INF = 1e18;
const ll MXN = 10, MXM = 40;

ll n, m;
struct edge {
    ll u, v, w[2];
    bool tg;
    ll getw() const { return w[tg]; }
} e[MXM];
...
ll cal() {
    auto cmp = [](ll x, ll y) { return e[x].getw() < e[y].getw(); };
    auto find = [](ll x) {
        while (fa[x] != x) x = fa[x] = fa[fa[x]];
        return x;
    };
    sort(ei + 1, ei + 1 + m, cmp);
    for (ll i = 1; i <= n; i++) fa[i] = i;
    ll ans = 0;
    for (ll i = 1; i <= m; i++) {
        edge &ce = e[ei[i]];
        ll x = find(ce.u), y = find(ce.v);
        if (x != y) {
            fa[x] = y;
            ans += ce.getw();
        }
...
const db T0 = 1e18, Tt = 0.001, rate = 0.99;
const ll solvet = 30;

ll solve(ll k) {
    ll ans = -INF;
    for (ll l = 1; l <= solvet; l++) {
        shuffle(e + 1, e + 1 + m, mr);
        for (ll i = 1; i <= m; i++) e[i].tg = i <= k;
        ll cur = cal(), nx;
        umx(ans, cur);
        if (k == 0 || k == m) break;
        for (db T = T0; T > Tt; T *= rate) {
            ll x = ri(1, m), y = ri(1, m);
            while ((e[x].tg ^ e[y].tg) == 0) y = ri(1, m);
            e[x].tg ^= 1, e[y].tg ^= 1;
            nx = cal();
            if (nx < cur && rd(0, 1) > exp((nx - cur) / T0))
                e[x].tg ^= 1, e[y].tg ^= 1;
            else
                cur = nx;
            umx(ans, cur);
...
int main() {
    ...
    while (t--) {
        ...
        for (ll i = 1; i <= m; i++) {
            ...
            /* cout<<ri(1,100)<<" "; */
            /* cout<<endl; */
            cin >> e[i].u >> e[i].v >> e[i].w[0] >> e[i].w[1];
            ei[i] = i;
        }
        for (ll i = 0; i <= m; i++) cout << solve(m - i) << endl;
    }
    return 0;
}
