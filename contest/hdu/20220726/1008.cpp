...
struct p {
    ll x, y, z;
    ll operator*(const p &b) const { return x * b.x + y * b.y + z * b.z; }
...
int main() {
    ...
    while (t--) {
        cin >> n;
        for (ll i = 1; i <= n; i++) {
            cin >> arr[i * 2 - 1].x >> arr[i * 2 - 1].y >> arr[i * 2 - 1].z;
            cin >> arr[i * 2].x >> arr[i * 2].y >> arr[i * 2].z;
        }
        bool f = 0;
        auto getn = [](p a, p b) {
            p res = p{a.y * b.z - a.z * b.y, a.z * b.x - a.x * b.z, a.x * b.y - a.y * b.x};
            assert(a * res == 0 && b * res == 0);
            return res;
        };
        ...
        for (ll i = 1; i <= n * 2; i++)
            for (ll j = i + 1; j <= n * 2; j++)
                for (ll k = j + 1; k <= n * 2; k++) {
                    p nvec = getn(arr[j] - arr[i], arr[k] - arr[i]);
                    if (nvec.x || nvec.y || nvec.z) {
                        f = 1;
                        ll cnt = 0;
                        for (ll l = 1; l <= n * 2; l += 2) {
                            ll r1 = nvec * (arr[l] - arr[i]);
                            ll r2 = nvec * (arr[l + 1] - arr[i]);
                            cnt += (r1 == 0 || r2 == 0 || (r1 > 0) ^ (r2 > 0));
                        }
                        umx(ans, cnt);
                    }
                }
