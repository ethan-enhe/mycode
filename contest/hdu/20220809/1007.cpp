...
ll n, wf[MXN], w[MXN], c[MXN], fa[MXN];
vector<pi> g[MXN];
vector<ll> son[MXN];
pi dp[MXN];
void init(ll p) {
...
bool cmp(ll x, ll y) { return wf[x] < wf[y]; }
void dfs(ll p) {
    for (auto &nx : g[p])
        if (nx.fi != fa[p]) {
            dfs(nx.fi);
            son[p].push_back(nx.fi);
        }
    son[p].push_back(p);
    son[p].push_back(0);
    wf[0] = w[p];
    ...
    sort(all(son[p]), cmp);
    ll cost = 0, last = -1, _cost = 0;
    pi tmp = {INF, INF};
    for (ll nx : son[p]) cost += dp[nx].se;
    for (ll nx : son[p]) {
        if (wf[nx] != last) _cost = cost;
        cost = cost - dp[nx].se + dp[nx].fi;
        _cost = _cost - dp[nx].se + min(dp[nx].fi, dp[nx].se);
        ll tc = _cost + abs(w[p] - wf[nx]) * c[p];
        /* if (p == 1) cout << nx << " " << wf[nx] << " " << cost << " " << _cost << nl; */
        if (wf[nx] <= wf[p]) umn(tmp.fi, tc);
        if (wf[nx] >= wf[p]) umn(tmp.se, tc);
        last = wf[nx];
    }
    dp[p] = tmp;
    son[p].clear();
