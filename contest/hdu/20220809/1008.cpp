...
ll cal(ll x, ll y, ll z) {
    if (x + y <= z || y + z <= x || x + z <= y) return INF;
    if (sg[x][y][z] == -1) {
        mex tmp;
        for (ll i = 1; i <= x; i++) tmp.ins(cal(x - i, y, z));
        for (ll i = 1; i <= y; i++) tmp.ins(cal(x, y - i, z));
        for (ll i = 1; i <= z; i++) tmp.ins(cal(x, y, z - i));
        sg[x][y][z] = tmp.ind;
    }
    return sg[x][y][z];
}
...
int main() {
    ...
    for (ll i = 1; i <= n; i++)
        for (ll j = i; j <= n; j++)
            for (ll k = j; k <= n; k++)
                if (i + j > k) {
                    if (cal(i, j, k) == 0) cout << i << " " << j << " " << k << " " << cal(i, j, k) << nl;
                }
