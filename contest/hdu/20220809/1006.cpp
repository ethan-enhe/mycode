...
mod solve(ll x) {
        ...
        for (ll j = 0; j <= i; j++)
            for (ll f = 0; f <= 1; f++)
                for (ll g = 0; g <= 1; g++) {
                    mod &cur = dp[i][j][f][g];
                    if (!cur.v) continue;
                    dp[i + 1][j + (d == 0 && !g)][f & (nxd == 0)][g] += cur;
                    if (nxd < d) {
                        if (nxd != 0) {
                            dp[i + 1][j][f][0] += cur;
                            dp[i + 1][j][0][0] += cur * (nxd - 1);
                        }
                        if (!f) {
                            dp[i + 1][j + 1][0][0] += cur;
                            dp[i + 1][j][0][0] += cur * (b - nxd - 2);
                        }
                    } else if (nxd == d) {
                        if (nxd != 0) {
                            dp[i + 1][j + 1][f][0] += cur;
                        ...
                        if (!f) dp[i + 1][j][0][0] += cur * (b - nxd - 1);
                    } else {
                        if (!f) dp[i + 1][j][0][0] += cur * (b - nxd - 1);
                        if (d != 0) dp[i + 1][j + 1][0][0] += cur;
                        dp[i + 1][j][f][0] += cur;
                        dp[i + 1][j][0][0] += cur * (nxd - 1 - (d != 0));
                    }
                }
    }
    mod ans = 0;
    for (ll i = 1; i <= ind; i++) {
        mod tmp = 0;
        for (ll f = 0; f <= 1; f++)
            for (ll g = 0; g <= 1; g++) tmp += dp[ind][i][f][g];
        ans += qpow(mod(i), k) * tmp;
        /* cout << (tmp) << endl; */
    }
    return ans;
}
