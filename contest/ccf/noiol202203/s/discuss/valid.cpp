...
bool chk(ll x, ll y) {
    set<ll> sx, sy;
    for (ll i : prob[x]) sx.insert(i);
    for (ll i : prob[y]) sy.insert(i);
    bool cap = 0, ny = 0, nx = 0;
    for (ll i : sx) {
        if (sy.count(i))
            cap = 1;
        else
...
int main() {
    ...
    cin.tie(0);
    ifstream in, ans;
    in.open("discuss.in", ios::in);
    ans.open("discuss.out", ios::in);
    int t;
    in >> t;
    ll yes = 0, no = 0;
    while (t--) {
