...
ll n, m, mxi[MXN], cnt[MXN];
vec<ll> prob[MXN];
ll p[MXN];
bool cmp(ll x, ll y) { return cnt[x] < cnt[y]; }
int main() {
    ...
    while (t--) {
        ...
        for (ll i = 1; i <= n; i++) {
            mxi[i] = 0;
            p[i] = i;
            prob[i].resize(cnt[i] = nxt());
            generate(all(prob[i]), nxt<ll>);
        }
        sort(p + 1, p + 1 + n, cmp);
        bool f = 0;
        for (ll tmp = 1; tmp <= n; tmp++) {
            /* cerr << p[tmp] << " "; */
            ll i = p[tmp];
            for (ll j : prob[i]) --cnt[mxi[j]];
            for (ll j : prob[i])
                if (mxi[j] && cnt[mxi[j]]) {
                    cout << "YES" << nl;
                    cout << i << " " << mxi[j] << nl;
