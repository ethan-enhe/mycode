...
int main() {
    ...
    ll ans = 0;
    for (ll j = 0; j < LG; j++) en[j] = n;
    for (ll i = n; i; i--) {
        ll mn = n;
        for (ll j = 0; j < LG; j++) {
            if (i == n || ((arr[i] ^ arr[i + 1]) & (1ll << j))) {
                if (j && !((arr[i] ^ arr[i + 1]) & (1ll << (j - 1)))) umn(en[j], i);
                if (cnt[j] != (1ll << j)) umn(en[j], i + cnt[j]);
                cnt[j] = 0;
            }
            ++cnt[j];
            if (cnt[j] > (1ll << j)) umn(en[j], i + (1ll << j) - 1);
            umn(mn, en[j]);
        }
        ans += mn - i + 1;
    }
