...
vector<ll> pfac[MXN];
bool sta[MXN];
ll mnfac[MXN], pri[MXN], pric;
ll cnt[MXN];

void delt(ll i, ll j, ll v, ll tp) {
    if (j == pfac[i].size()) {
        cnt[v] += tp;
        return;
    }
    delt(i, j + 1, v, tp);
    delt(i, j + 1, v * pfac[i][j], tp);
}
ll cal(ll i, ll j, ll v, ll tp) {
    if (j == pfac[i].size()) return cnt[v] * tp;
    return cal(i, j + 1, v, tp) + cal(i, j + 1, v * pfac[i][j], -tp);
}
