...
void ins(ll x) {
    tmp.push_back(x);
    ll ind = tmp.size() - 1;
    while (ind && tmp[ind] < tmp[ind - 1]) {
        swap(tmp[ind], tmp[ind - 1]);
        --ind;
    }
}
ll get() {
    ll ind = tmp.size() - ll(0.05 * tmp.size());
    return tmp[ind - 1];
}
void clr() { tmp.clear(); }
ll va[MXN][MXN], dp[MXN][MXN];
int main() {
    ...
    while (t--) {
        ...
        for (ll i = 1; i <= m; i++)
            for (ll j = 1; j <= n; j++)
                for (ll k = j - 1; ~k; k--) umn(dp[i][j], dp[i - 1][k] + va[k + 1][j]);
        cout << dp[m][n] << nl;
    }
    return 0;
}
