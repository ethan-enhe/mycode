...
const char nl = '\n';
const ll INF = 1e18;
const ll MXN = 755, MXM = 155, MXA = 21;

ll n, m, arr[MXN];
ll dp[MXN][MXM][MXA][MXA];
int main() {
        ...
        for (ll j = 0; j <= m; j++)
            for (ll cur = 1; cur < MXA; cur++)
                for (ll nx = 0; nx < MXA; nx++) {
                    if (dp[i][j][cur][nx])
                        cout << i << " " << j << " " << cur << " " << nx << " " << dp[i][j][cur][nx] << nl;
                    for (ll ca = l; ca <= r; ca++) {
                        ll _nx = max(nx, ca - cur + 1);
                        if (cur > 1)
                            dp[i + 1][j][cur - 1][_nx] = (dp[i + 1][j][cur - 1][_nx] + dp[i][j][cur][nx]) % P;
                        else
                            dp[i + 1][j + 1][_nx][0] = (dp[i + 1][j + 1][_nx][0] + dp[i][j][cur][nx]) % P;
                    }
                }
    }
    ll ans = 0;
    for (ll cur = 1; cur < MXA; cur++)
        for (ll nx = 0; nx < MXA; nx++) ans = (ans + dp[n + 1][m][cur][nx]) % P;
    cout << ans;
