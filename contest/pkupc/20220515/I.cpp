...
pair<char, ll> tmp[MXN];
void clr() {
    ll _m = 0;
    for (ll i = 1; i <= m; i++)
        if (tmp[i].se) tmp[++_m] = tmp[i];
    m = _m;
}
void mrg() {
    ll _m = 0;
    for (ll i = 1; i <= m; i++) {
        if (tmp[_m].fi == tmp[i].fi)
            tmp[_m].se += tmp[i].se;
        else
            tmp[++_m] = tmp[i];
    }
    m = _m;
    if (tmp[m].fi == tmp[1].fi) tmp[1].se += tmp[m--].se;
}
int main() {
    ...
    if (cnt != 0) return cout << "Impossible", 0;
    ll ans = 1;
    tmp[0].fi = '?';
    mrg();
    while (m) {
        for (ll i = 1; i <= m; i++) --tmp[i].se;
        clr();
        if (!m) mrg();
        ++ans;
    }
