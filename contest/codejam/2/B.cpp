...
const char nl = '\n';
const ll INF = 1e18;
const ll MXN = 220, ZERO = 110;

ll n, m;
bool a[MXN][MXN], b[MXN][MXN];
void draw(ll r, bool f[MXN][MXN]) {
    for (ll x = -r; x <= r; x++) {
        ll y = round(sqrt(r * r - x * x));
        f[ZERO + x][ZERO + y] = f[ZERO + x][ZERO - y] = f[ZERO + y][ZERO + x] = f[ZERO - y][ZERO + x] = 1;
    }
}
void fill(ll r, bool f[MXN][MXN]) {
    for (ll x = -r; x <= r; x++)
        for (ll y = -r; y <= r; y++)
            if (round(sqrt(x * x + y * y)) <= r) f[x + ZERO][y + ZERO] = 1;
}
int main() {
    ...
    while (t--) {
        ...
        cin >> n;
        fill(n, b);
        for (ll r = 0; r <= n; r++) draw(r, a);
        ll ans = 0;
        for (ll x = -n; x <= n; x++)
            for (ll y = -n; y <= n; y++) ans += (a[x + ZERO][y + ZERO] ^ b[x + ZERO][y + ZERO]);
        cout << "Case #" << task << ": " << ans << nl;
    }
    return 0;
}
