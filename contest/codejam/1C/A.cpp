...
const char nl = '\n';
const ll INF = 1e18;
const ll MXN = 1e6 + 5, C = 256;

ll n, m, arr[MXN];
string out[C], cur[C];
bool hasin[C];
string calc(int x) {
    if (out[x].empty()) return cur[x];
    return cur[x] + out[x] + calc(out[x].back());
}
bool app[C];
bool chk(string s) {
    memset(app, 0, sizeof(app));
    for (ll i = 0; i < s.size(); i++) {
        if (app[s[i]] && s[i] != s[i - 1]) return 0;
        app[s[i]] = 1;
    }
    return 1;
}
int main() {
    ...
    for (ll task = 1; task <= t; task++) {
        //联通，11进1出
        for (ll i = 0; i < C; i++) out[i] = cur[i] = "", hasin[i] = 0;
        m = 0;
        cin >> n;
        bool f = 0;
        for (ll i = 1; i <= n; i++) {
            ...
            if (tmp.front() == tmp.back())
                ...
                if (!out[tmp.front()].empty()) f = 1;
                out[tmp.front()] = tmp;
                if (hasin[tmp.back()]) f = 1;
                hasin[tmp.back()] = 1;
            }
        }
        cout << "Case #" << task << ": ";
        if (f)
            ...
            string ans;
            for (ll i = 0; i < C; i++)
                if (!out[i].empty() && !hasin[i]) ans += calc(i);
            for (ll i = 0; i < C; i++)
                if (!hasin[i] && out[i].empty()) ans += cur[i];
            if (ans.size() == m && chk(ans))
                cout << ans;
            else
                cout << "IMPOSSIBLE";
        }
