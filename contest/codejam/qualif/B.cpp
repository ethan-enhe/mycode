...
int main() {
    ...
    int t;
    cin >> t;
    for (ll task = 1; task <= t; task++) {
        for (ll i = 1; i <= 4; i++) arr[i] = INF;
        for (ll i = 1; i <= 3; i++)
            for (ll j = 1; j <= 4; j++) umn(arr[j], nxt());
        ll sum = accumulate(arr + 1, arr + 5, 0);
        cout << "Case #" << task << ":";
        if (sum < MXN)
            cout << " IMPOSSIBLE\n";
        else {
            for (ll j = 1; j <= 4; j++) {
                if (sum > MXN) {
                    ll delt = min(arr[j], sum - MXN);
                    arr[j] -= delt;
                    sum -= delt;
                }
            }
            for (ll j = 1; j <= 4; j++) cout << " " << arr[j];
            cout << nl;
        }
    }
    return 0;
