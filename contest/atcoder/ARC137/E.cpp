...
struct PushRelabelMinCostMaxFlow {
    ...
    PushRelabelMinCostMaxFlow(flowUnit FLOW_EPS, costUnit COST_INF, costUnit COST_EPS)
        : FLOW_EPS(FLOW_EPS), COST_INF(COST_INF), COST_EPS(COST_EPS) {}
    ...
    pair<flowUnit, costUnit> getMaxFlowMinCost(int V, int s = -1, int t = -1) {
        ...
        while (bnd > 1) {
            bnd /= SCALE;
            bnd = max(bnd, costUnit(1));
            top = 0;
            for (int v = 0; v < V; v++)
                for (auto &&e : adj[v])
                    if (costP(v, e) < -COST_EPS && e.resCap > FLOW_EPS) push(v, e, e.resCap, false);
            for (int v = 0; v < V; v++)
                if (ex[v] > FLOW_EPS) stk[top++] = v;
...
PushRelabelMinCostMaxFlow<MXN, ll, ll> mcmf(0, INF, 0);
ll n, m, d;
int main(int argc, char *argv[]) {
    cin >> n >> m >> d;
    for (ll i = 1, x; i <= n; i++) {
        cin >> x;
        mcmf.addEdge(i - 1, i, x, -d);
        mcmf.addEdge(i - 1, i, MXN, 0);
    }
    for (ll i = 1; i <= m; i++) {
        ll l, r, c;
        cin >> l >> r >> c;
        mcmf.addEdge(r, l - 1, 1, c);
    }
    cout << -mcmf.getMaxFlowMinCost(n + 1).second;
    return 0;
}
