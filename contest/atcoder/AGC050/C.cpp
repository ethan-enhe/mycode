...
ll n, m, arr[MXN];
mod dp[LG][MXN]; // 后面有i个B,当前算到j，最前一个B恰好为i
ll pw[MXN], mv[MXN];
ll cont[MXN];
char *str, tmp[MXN];
int main() {
    ...
    pw[0] = 2;
    for (ll i = 1; i < LG; i++) {
        pw[i] = pw[i - 1] * 2 - 1; // 要在Bi次除2后还没死，至少需要多大空间（包括左右自己）
    }
    mv[0] = 0;
    for (ll i = 1; i < LG; i++) {
        mv[i] = pw[i - 1] - 1;
    ...
    for (ll i = 0; i < LG; i++) {
        for (ll j = n; j; j--) {
            if (i && (str[j] == '?' || str[j] == 'B')) {
                if (j + mv[i - 1] <= n && mv[i - 1] <= cont[j + 1]) dp[i][j] += dp[i - 1][j + mv[i - 1] + 1];
            }
            if (str[j] == '?' || str[j] == 'S') {
                dp[i][j] += dp[i][j + 1];
            }
    ...
    mod ans = qpow((mod)2, cnt);
    for (int i = 0; i < LG; i++) ans -= dp[i][1];
    cout << ans;
    return 0;
}
