...
vector<node> qry;
ll s[MXN], t[MXN], ans[MXN];

void solve(ll l, ll r, vector<node> &ask) {
    if (l == r) {
        for (auto &i : ask) ans[i.id] = l;
        return;
    }
    ll mid = (l + r) >> 1, osz = op.size();
    for (ll i = l; i <= mid; i++) mrg(s[i], t[i]);
    vector<node> lask, rask;
    for (auto &i : ask) {
        if (mxvis(i.x, i.y) < i.z) {
            rask.push_back(i);
        } else
            lask.push_back(i);
    }
...
int main() {
    ...
    cin.tie(0);
    cin >> n >> m;
    for (ll i = 1; i <= n; i++) rnd[i] = ri(1, INF), sz[i] = 1, fa[i] = i;
    for (ll i = 1; i <= m; i++) {
        cin >> s[i] >> t[i];
    }
    cin >> q;
