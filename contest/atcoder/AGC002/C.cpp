...
ll sum[MXN];
void upd(ll x, ll y) {
    knot.erase(knot.find({sum[x], x}));
    knot.insert({(sum[x] = y), x});
}
...
int main() {
    ...
    for (ll i = 1; i <= n; i++) {
        ...
        cin >> x;
        st.insert({i, x});
        if (i > 1) knot.insert({(sum[i] = x + last), i});
        last = x;
    }
    stack<ll> ans;
    ...
    while (!knot.empty()) {
        ...
        auto its = st.lower_bound({ind, 0});
        auto pre = prev(its);
        pi newnode = {pre->fi, pre->se + its->se};
        st.erase(pre);
        st.erase(st.lower_bound({ind, 0}));
        its = st.insert(newnode).first;
        if (its != st.begin()) {
            pre = prev(its);
            upd(its->fi, its->se + pre->se);
        }
        auto suf = next(its);
        if (suf != st.end()) upd(suf->fi, its->se + suf->se);
    }
    cout << "Possible\n";
    while (!ans.empty()) {
        cout << ans.top() << nl;
