...
ll n, x, y;
vector<ll> rg[MXN], bg[MXN];
ll bfa[MXN], bdis[MXN];
bool win[MXN];
bool bdisle2(ll x, ll y) {
    if (x == y) return 1;
    if (y == bfa[x]) return 1;
    if (x == bfa[y]) return 1;
    if (bfa[x] == bfa[y]) return 1;
    if (bfa[bfa[x]] == y) return 1;
    if (bfa[bfa[y]] == x) return 1;
    return 0;
}
void bdfs(ll p) {
    for (ll nx : bg[p])
...
void rdfs(ll p, ll fa) {
    for (ll nx : rg[p])
        if (nx != fa) {
            if (!bdisle2(p, nx)) win[p] = win[nx] = 1;
            rdfs(nx, p);
        }
}
ll ans = -1;
void solve(ll p, ll fa, ll dpth) {
    if (dpth < bdis[p]) {
        if (win[p]) ans = INF;
        ans = max(bdis[p] * 2, ans);
    } else {
        ans = max(bdis[p] * 2, ans);
        return;
    }
    for (ll nx : rg[p])
        if (nx != fa) solve(nx, p, dpth + 1);
}
int main() {
