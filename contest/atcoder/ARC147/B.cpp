...
ll n, m, arr[MXN];
bool notok(ll i) { return (arr[i] ^ i) & 1; }
vector<pi> op;
void op2(ll i) {
    op.push_back({2, i});
    swap(arr[i], arr[i + 2]);
...
void st(ll init) {
    ...
    while (f) {
        f = 0;
        for (ll i = init; i + 2 <= n; i += 2)
            if (arr[i] > arr[i + 2]) op2(i), f = 1;
    }
}
int main() {
    ...
    cout << op.size() << endl;
    for (auto i : op) {
        cout << (i.fi == 1 ? "A " : "B ") << i.se << nl;
    }
    for (ll i = 1; i <= n; i++) assert(arr[i] == i);
    return 0;
}
