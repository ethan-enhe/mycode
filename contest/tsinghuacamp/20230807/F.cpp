...
ll n, m, r[MXN], t[MXN], p[MXN], pool[MXN];
int record[MXN][MXN];
pi dp[MXN][MXN];
pi tmp[MXN];
pi cur[MXN];
...
void qry(ll x, pi *k) {
    for (ll i = 0; i <= m; i++) k[i] = {INF, 0};
    for (; x <= n; x += x & (-x)) {
        for (ll i = 0; i <= m; i++) umn(k[i], dp[x][i]);
    }
}
void upd(ll x, pi *k) {
    for (; x; x -= x & (-x)) {
        for (ll i = 0; i <= m; i++) umn(dp[x][i], k[i]);
        // cout<<x<<endl;
    }
}
int main() {
    ...
    for (ll i = 1; i <= n; i++) {
        r[i] = lower_bound(pool + 1, pool + 1 + n, r[i]) - pool;
        // cout<<r[i];
    }
    for (int i = 1; i <= n; i++) {
        for (int j = 0; j <= m; j++) dp[i][j] = {INF, 0};
    }
    for (int j = 0; j <= m; j++) tmp[j] = {INF, 0};
    tmp[0] = {0, 0};
    upd(n, tmp);
    for (ll i = 1; i <= n; i++) {
        qry(r[i], tmp);
        for (ll j = 0; j <= m; j++) {
            cur[j] = {p[i] + tmp[max(0ll, j - t[i])].fi, i};
            record[i][j] = tmp[max(0ll, j - t[i])].se;
        }
        upd(r[i], cur);
    }
    qry(1, tmp);
