...
ll n, m, ind, arr[MXN], pool[MXN], dp[MXN];
ll mxl[MXN], mxr[MXN], mx[MXN];

void mod(ll id) {
    for (ll x = arr[id]; x; x -= x & (-x)) umx(mxr[x], dp[id] + id);
    for (ll x = arr[id]; x <= ind; x += x & (-x)) umx(mxl[x], dp[id] - id);
    umx(mx[arr[id]], dp[id]);
}
void upd(ll id) {
    dp[id] = -INF;
    for (ll x = arr[id] - 1; x; x -= x & (-x)) umx(dp[id], mxl[x] + id);
    for (ll x = arr[id] + 1; x <= ind; x += x & (-x)) {
        /* cerr << x << " " << mxr[x] << endl; */
        umx(dp[id], mxr[x] - id);
    }
    umx(dp[id], mx[arr[id]]);
...
int main() {
    ...
    while (t--) {
        ...
        pool[++ind] = 0;
        /* cerr << ind << endl; */
        /* for (ll i = 1; i <= ind; i++) cerr << pool[i] << " "; */
        /* cerr << endl; */
        sort(pool + 1, pool + 1 + ind);
        ind = unique(pool + 1, pool + 1 + ind) - pool - 1;
        for (ll i = 0; i <= n; i++) arr[i] = lower_bound(pool + 1, pool + 1 + ind, arr[i]) - pool;
        for (ll i = 1; i <= ind; i++) mxl[i] = mxr[i] = mx[i] = -INF;
        for (ll i = 0; i <= n; i++) {
            if (i) upd(i);
            mod(i);
        }
