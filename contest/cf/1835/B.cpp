...
ll n, m, k, arr[MXN];
ll cnt[MXN]; // i~i-1;
ll rb(ll l, ll nx) { return min(m, (nx + l) >> 1); }
ll lb(ll prev, ll r) {
    // 2r>rr+prev
    return max(0ll, ((r + prev) >> 1) + 1);
}
int main() {
    ...
    if (k > 1)
        for (ll i = k - 1; i <= n; i++) {
            ll tmp = rb(arr[i - k + 2], arr[i + 1]) - lb(arr[i - k], arr[i]) + 1;
            cnt[i - k + 3] += tmp;
            cnt[i + 1] -= tmp;
            // cout << i << " " << rb(arr[i - k + 1], arr[i + 1]) << endl;
        }
    partial_sum(cnt, cnt + n, cnt);
    ll mx = -1, mxi = -1;
    for (ll i = 1; i <= n + 1; i++) {
        // cout << cnt[i] << endl;
        for (ll j = arr[i - 1] + 1; j < min(arr[i], arr[i - 1] + 10); j++) {
            // 右侧
            ll tmp = cnt[i];
            if (i + k - 2 <= n) {
                tmp += rb(j, arr[i + k - 1]) - lb(arr[i - 1], arr[i + k - 2]) + 1;
            }
            if (i >= k) {
                tmp += rb(arr[i - k + 1], arr[i]) - lb(arr[i - k], j) + 1;
                // if (j == 3) cout << rb(arr[i - k + 1], arr[i]) << " " << lb(arr[i - k], j) << endl;
                // if (j == 3) cout << rb(j, arr[i + k - 1]) << " " << lb(arr[i - 1], arr[i + k - 2]) + 1 << nl;
            }
            if (tmp > mx)
                mx = tmp, mxi = j;
            else if (tmp == mx)
