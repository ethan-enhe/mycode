...
ll n, m, arr[MXN];
ll pw10[] = {1, 10, 100, 1000, 10000, 100000, 1000000, 10000000};
int main() {
    ...
    while (t--) {
        ...
        --c;
        bool f = 1;
        for (ll i = pw10[a]; i < pw10[a + 1]; i++) {
            ll l = max(pw10[b], pw10[c] - i);
            ll r = min(pw10[b + 1] - 1, pw10[c + 1] - 1 - i);
            ll cnt = max(0ll, r - l + 1);
            if (d > cnt) {
                d -= cnt;
            } else {
                cout << i << " + " << l + d - 1 << " = " << (i + (l + d - 1)) << nl;
                f = 0;
                break;
            }
        }
