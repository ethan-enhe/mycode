...
int main(int argc, char *argv[]) {
    ...
    while (t--) {
        read(n);
        for (int i = 1; i <= n * 2; i++)
            for (int j = 1; j <= n * 2; j++) read(arr[i][j]);
		ll tot = 0;
        for (int i = n + 1; i <= n * 2; i++)
            for (int j = n + 1; j <= n * 2; j++) {
                tot += arr[i][j];
                // arr[i][j] = arr[i-n][j-n]=INF;
            }
		ll ans=INF;
		for(int i=0;i<2;i++)
			for(int j=0;j<2;j++){
				umn(ans,arr[1+i*(n-1)][n+1+j*(n-1)]);
				umn(ans,arr[n+1+i*(n-1)][1+j*(n-1)]);
			}

		prt(ans+tot,'\n');

		// dij();
		// prt(tot+min({dis[n+1][n],dis[n][n+1],dis[n][n*2],dis[n*2][n]}),"\n");
    }
