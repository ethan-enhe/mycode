...
void solve(ll r) {
    ...
    for (ll i = 1; i <= n; i++) {
        if (st_point > i) {
            if (str[i] == '0' && str[i - (n - r)] == '1') st_point = i;
        }
        tmp[i] = '0' + ((str[i] == '1') ^ ((st_point <= i) && (str[i - (n - r)] == '1')));
    }
    if (strcmp(tmp + 1, best + 1) > 0) {
        memcpy(best + 1, tmp + 1, n);
        bestl = r - (n - st_point), bestr = r;
    }
}
...
int main() {
    ...
    while (t--) {
        ...
        str[0] = str[n + 1] = 0;
        bestl = bestr = 0;
        for (ll i = 1; i <= n; i++) best[i] = 0;
        best[n + 1] = tmp[n + 1] = 0;
        // solve(3);
        for (ll i = 1; i <= n; i++) solve(i);
        cout << 1 << " " << n << " " << bestl << " " << bestr << nl;
    }
    return 0;
}
