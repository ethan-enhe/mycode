...
vector<ll> solve(ll l, ll r) { // 区间内，操作i次，最大值最小是多少（i<=60）
    // cerr << l << ' ' << r << nl;
    if (l > r) return vector<ll>(LOGINF, 1);
    auto [val, pos] = getmn(l, r);
    vector<ll> resl = solve(l, pos - 1), resr = solve(pos + 1, r);
    vector<ll> res(LOGINF, INF);
    for (ll i = 0; i < LOGINF; i++)
        for (ll j = 0; j <= i; j++) {
            res[i] = min(res[i], max({resl[j], resr[i - j], arr[pos]}));
        }
    for (ll i = 1; i < LOGINF; i++) {
        res[i] = min(res[i], (res[i - 1] + val - 1) / val);
    }
    return res;
}
int main() {
