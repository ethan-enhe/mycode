...
int main() {
    ...
    while (t--) {
        ...
        iota(ord, ord + C, 0);
        sort(ord, ord + C, [](ll &x, ll &y) { return cnt[x] > cnt[y]; });
        ll mn = INF, mni = -1;
        for (ll i = 1; i <= C; i++)
            if (n % i == 0) {
                ll cur = 0;
                for (ll j = 0; j < i; j++) cur += abs(n / i - cnt[ord[j]]);
                for (ll j = i; j < C; j++) cur += abs(cnt[ord[j]]);
                // if (i == 5) cerr << " " << cur << endl;
                if (cur < mn) {
                    mn = cur;
                    mni = i;
                }
        ...
        for (ll i = mni; i < C; i++) {
            while (!ind[ord[i]].empty()) {
                tmp.push(ind[ord[i]].top());
                ind[ord[i]].pop();
            }
        }
        for (ll i = 0; i < mni; i++)
            while ((ll)ind[ord[i]].size() > n / mni) {
                tmp.push(ind[ord[i]].top());
                ind[ord[i]].pop();
            }
        for (ll i = 0; i < mni; i++)
            while ((ll)ind[ord[i]].size() < n / mni) {
                ind[ord[i]].push(tmp.top());
                tmp.pop();
            }
        cout << mn / 2 << nl;
        for (ll i = 0; i < C; i++) {
