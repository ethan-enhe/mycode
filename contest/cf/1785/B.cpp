...
char ch(ll x) { return "win"[x]; }
vector<tuple<ll, ll, ll, ll>> ans;
void swp(ll a, ll b, ll c, ll d) {
    --cnt[a][b], ++cnt[a][d];
    --cnt[c][d], ++cnt[c][b];
    ans.push_back(tie(a, b, c, d));
}
stack<ll> multi0, multi1;
int main() {
    ...
    while (t--) {
        ...
        for (ll i = 1; i <= n; i++) {
            ...
            char str[10];
            cin >> str;
            for (ll j = 0; j < 3; j++) ++cnt[i][conv(str[j])];
        }
        while (!multi0.empty()) multi0.pop();
        while (!multi1.empty()) multi0.pop();
        for (ll i = 1; i <= n; i++) {
            if (cnt[i][0] > 1 && cnt[i][1] == 0) multi0.push(i);
            if (cnt[i][1] > 1 && cnt[i][0] == 0) multi1.push(i);
        }
        while (!multi0.empty() && !multi1.empty()) {
            swp(multi0.top(), 0, multi1.top(), 1);
            multi0.pop();
            multi1.pop();
        }
        while (!multi0.empty()) multi0.pop();
        ...
        for (ll i = 1; i <= n; i++)
            if (cnt[i][2]) {
                if (!cnt[i][0]) {
                    assert(!multi0.empty());
                    swp(multi0.top(), 0, i, 2);
                    multi0.pop();
                }
                if (!cnt[i][1]) {
                    assert(!multi1.empty());
                    swp(multi1.top(), 1, i, 2);
                    multi1.pop();
                }
            }
        cout << ans.size() << nl;
