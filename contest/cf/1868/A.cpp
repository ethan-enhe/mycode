...
ll n, m, arr[MXN];
ll get(ll x, ll y) { return (n == 1 ? 0 : x % (n - 1) + y) % n; }
bool f[MXN];
ll idx;
void ins(ll x) {
    f[x] = 1;
...
int main() {
    ...
    while (t--) {
        cin >> m >> n;
        for (ll i = 0; i < n; i++) {
            for (ll j = 0; j < m; j++) ins(get(j, i));
            arr[i] = idx;
            idx = 0;
            for (ll j = 0; j < m; j++) f[get(j, i)] = 0;
        }
        for (ll i = 0; i < n; i++) {
            ins(arr[i]);
        }
        ll tmp = idx;
        idx = 0;
        for (ll i = 0; i <= n; i++) f[arr[i]] = 0;
        cout << tmp << nl;
        for (ll i = 0; i < m; i++) {
            for (ll j = 0; j < n; j++) cout << get(i, j) << " ";
            cout << nl;
        }
    }
    return 0;
