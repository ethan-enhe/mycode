...
ll solve(ll l, ll r, ll lw, ll rw, ll fl) {
    if (l > r) return 0;
    ll &cdp = dp[l][r][lw][rw][fl];
    if (cdp == -1) {
        mex ans;
        for (ll i = l; i <= r; i++)
            if ((str[i] == 'R') ^ fl) {
                if (i != l || lw) ans.ins(solve(l, i - 2, lw, 1, !fl) ^ solve(i + 1, r, 1, rw, !fl));
                if (i != r || rw) ans.ins(solve(l, i - 1, lw, 1, !fl) ^ solve(i + 2, r, 1, rw, !fl));
            }
        cdp = ans.ind;
        cout << l << " " << r << " " << lw << " " << rw << " " <<fl<<" "<< cdp << nl;
    }
    return cdp;
}
...
int main() {
    ...
    while (t--) {
        ...
        cin >> (str + 1);
        /* cout << (solve(1, n, 0, 0, 0) ? "Alice" : "Bob") << nl; */
        solve(1, n, 1, 1, 0);
        /* cout << solve(1, n, 1, 1, 0) << endl; */
        /* for (ll i = 1; i <= n; i++) cout << solve(1, i, 0, 0, 0) << nl; */
    }
    return 0;
}
