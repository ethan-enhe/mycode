...
ll n, m;
mod dp[MXN][2][2][2][2][2][2];
char str[MXN];
#define swt(x) for (ll x = 0; x <= 1; x++)
int main() {
    ...
    dp[0][1][1][1][1][1][0] = 1;
    for (ll i = 0; i < n; i++) {
        swt(f1) swt(f2) swt(f3) swt(g1) swt(g2) swt(tri) {
            mod &cdp = dp[i][f1][f2][f3][g1][g2][tri];
            if (cdp.v == 0) continue;
            ll tmp = str[i] - '0';
#define up(x) (tmp ? 1 : !(x))
            for (ll d1 = 0; d1 <= up(f1); d1++)
                for (ll d2 = 0; d2 <= up(f2); d2++)
                    for (ll d3 = 0; d3 <= up(f3); d3++) {
                        bool a = d2 ^ d3, b = d1 ^ d3, c = d1 ^ d2;
                        if (g1 && a < b) continue;
                        if (g2 && b < c) continue;
                        /* if (!g1 && !g2 && a < c) continue; */
                        dp[i + 1][f1 & (tmp == d1)][f2 & (tmp == d2)][f3 & (tmp == d3)][g1 & (a == b)][g2 & (b == c)]
                          [tri | (b == 1 && c == 1)] += cdp;
                    }
        }
    }
    ...
    swt(g1) swt(g2) {
        mod sum = 0;
        swt(f1) swt(f2) swt(f3) sum += dp[n][f1][f2][f3][g1][g2][1];
        if (g1 + g2 == 0)
            ans += sum * 6;
        else
            ans += sum * 3;
