...
int main() {
    ...
    for (ll i = 1; i <= n; i++)
        for (ll j = 1; j <= n; j++) mns[i] += min(i, cnt[j]);
    for (ll j = 1; j <= n; j++) dp[j][0] = 1;
    mod ans = 0;
    for (ll i = 1; i <= n; i++) {
        memset(dp[n / i + 1], 0, sizeof(mod) * MXN);
        for (ll j = n / i; j; j--) {
            for (ll k = n; k > mns[i]; k--) dp[j][k] = dp[j + 1][k];
            for (ll k = mns[i]; k >= 0; k--) dp[j][k] = dp[j + 1][k] + dp[j][k - j];
            for (ll k = j - 1; k; k--) dp[j][k] = dp[j + 1][k];
        }
        ans += dp[1][n];
        /* cout<<i<<" "<<dp[1][n]<<endl; */
    }
