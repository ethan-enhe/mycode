...
void mod(ll x, ll y) {
    for (ll i = 0; i < fac.size(); i++) {
        ll curind = x % (n / fac[i]);
        mxsum[i].erase({sum[i][curind], curind});
        sum[i][curind] += y - arr[x];
        mxsum[i].insert({sum[i][curind], curind});
    }
    arr[x] = y;
}
ll getmax() {
    ll ans = 0;
    for (ll i = 0; i < fac.size(); i++) umx(ans, mxsum[i].rbegin()->first * (n / fac[i]));
    return ans;
}
int main() {
    ...
    while (t--) {
        ...
        if (tn != 1) fac.push_back(tn);

        for (ll i = 0; i < fac.size(); i++) {
            sum[i].resize(n / fac[i]);
            mxsum[i].clear();
            for (ll j = 0; j < n / fac[i]; j++) mxsum[i].insert({sum[i][j] = 0, j});
        }
        for (ll i = 1, x; i <= n; i++) {
            cin >> x;
            arr[i] = 0;
