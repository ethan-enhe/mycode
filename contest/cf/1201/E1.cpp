...
#define ind(x) [(x).fi][(x).se]

ll deg[MXN][MXN][MXN][MXN];
bool haswin[MXN][MXN][MXN][MXN];
ll res[MXN][MXN][MXN][MXN];

bool inb(const pi &x) { return x.fi >= 1 && x.fi <= n && x.se >= 1 && x.se <= m; }
queue<pair<pi, pi>> q;
int main() {
