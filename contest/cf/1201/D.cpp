...
ll cal(ll x, ll y) {
    ...
    auto pre = prev(it);
    return min(abs((*it) - x) + abs((*it) - y), abs((*pre) - x) + abs((*pre) - y));
}
int main() {
    ...
    ll ans = 0;
    ll lmn = mn[1], lmx = mx[1];
    ll d0 = (mx[1] - mn[1]) * 2, d1 = mx[1] - mn[1];

    for (ll i = 2; i <= mxr; i++) {
        ++ans;
        if (mn[i] <= mx[i]) {
            ans += mx[i] - mn[i];
            ll nd0 = min(d0 + cal(lmn, mx[i]), d1 + cal(lmx, mx[i]));
            ll nd1 = min(d0 + cal(lmn, mn[i]), d1 + cal(lmx, mn[i]));
            d0 = nd0, d1 = nd1;
            lmn = mn[i], lmx = mx[i];
        }
    }
