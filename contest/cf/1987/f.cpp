...
bool valid[MXN];
ll arr[MXN];
ll dp[MXN][MXN]; // l-r，有(arr[l]-l)/2 个减2机会，最多能消几次（不能跨边界）
...
int main() {
    ...
    ll t;
    cin >> t;
    auto v = [&](ll i) { return i - arr[i]; };
    while (t--) {
        cin >> n;
        for (ll i = 1; i <= n; i++) {
            cin >> arr[i];
            valid[i] = v(i) >= 0 && (v(i) & 1) == 0;
        }
        for (ll i = 0; i <= n; i++)
            for (ll j = 0; j <= n; j++)
                for (ll k = 0; k <= n; k++) dp[i][j] = 0;
        valid[0] = 1;
        for (ll l = n; l; l--) {
            for (ll r = l; r <= n; r++) {
                if (!valid[l - 1]) continue;
                for (ll last = l; last < r; last++) {
                    if (!valid[last]) continue;
                    ll dl = dp[l][last - 1];
                    ll dr = dp[last + 1][r];
                    ll tmp = (v(last) - v(l - 1)) / 2;
                    if (dl < tmp) continue;
                    umn(dr, (r - last - 1) / 2); // 至少预留一个
                    umx(dp[l][r], dl + dr + 1);
                }
                // cout << l << " " << r << " " << dp[l][r] << nl;
            }
