...
ll n, m, arr[MXN];
mod fac_cnt[MXN], cnt[MXN];
ll mu[MXN];
void getmobius() {
    mu[1] = 1;
    static bool vis[MXN];
    static ll prime[MXN], tot;
    for (ll i = 2; i < MXN; i++) {
        if (!vis[i]) {
            prime[++tot] = i;
            mu[i] = -1;
        }
        for (ll j = 1; j <= tot && i * prime[j] < MXN; j++) {
            vis[i * prime[j]] = 1;
            if (i % prime[j] == 0) {
                mu[i * prime[j]] = 0;
                break;
    ...
    for (ll i = 1; i <= n; i++) {
        ...
        for (ll j = 1; j * j <= arr[i]; j++) {
            if (arr[i] % j == 0) {
                cnt[i] += fac_cnt[j];
                if (j * j != arr[i]) {
                    cnt[i] += fac_cnt[arr[i] / j];
                }
            }
        }
        for (ll j = 1; j * j <= arr[i]; j++) {
            if (arr[i] % j == 0) {
                fac_cnt[j] -= cnt[i] * mu[j];
                if (j * j != arr[i]) {
                    fac_cnt[arr[i] / j] -= cnt[i] * mu[arr[i] / j];
                }
            }
        }
        fac_cnt[1] += cnt[i];
