...
int main() {
    ...
    for (ll i = 1; i <= n; i++) {
        cin >> arr[i];
        if (arr[i] == 0) ans += i * (n - i + 1);
    }
    ll last_not_0 = n + 1;
    ll last = n + 1, lastlast = n + 1;
    ll last_leak_2 = n + 1, last_leak_1 = n + 1;
    for (ll i = n; i; i--) {
        if (arr[i]) last_not_0 = i;
        if (arr[i] != 3 && arr[i]) {
            if (arr[i] != arr[last]) lastlast = last;
            last = i;
        }

        //>=3
        ll mn3 = max(last_leak_1, last_leak_2);
        if (arr[lastlast] == 1) mn3 = last_leak_2;
        if (arr[lastlast] == 2) mn3 = last_leak_1;
        ans += n - mn3 + 1;
        ...
        if (arr[lastlast])
            ...
            ans += (n - min(last_leak_2, last_leak_1) + 1);
        // cerr << ans << endl;
        ...
        if (arr[i] == 3) {
            if (arr[lastlast] == 1)
                last_leak_1 = lastlast;
            else if (arr[lastlast] == 2)
                last_leak_2 = lastlast;
            last = lastlast = n + 1;
        }
    }
    cout << ans;
