...
int main() {
    ...
    while (m--) {
        ...
        f.addedge(u, v, c >> 1, w << 1, 1);
    }
    int add = 0;
    for (int i = 1; i <= n; i++) {
        if (deg[i] & 1) goto notok;
        // cerr << i << " " << (deg[i] / 2) << endl;
        if (deg[i] < 0) f.addedge(i, st, 0, (-deg[i]) >> 1, 0, 1);
        if (deg[i] > 0) {
            f.addedge(ss, i, 0, deg[i] >> 1, 0, 1);
            add += (deg[i] >> 1);
        }
    }
    res = f.run(MXN - 3, MXN - 4, ss, st, 1);
    // if (res.first != add) goto notok;
    cout << "Possible";
