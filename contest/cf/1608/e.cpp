...
bool cmpx(const point &x, const point &y) { return x.x[0] < y.x[0]; }
bool cmpy(const point &x, const point &y) { return x.x[1] < y.x[1]; }
...
bool chk2(ll k, ll c1, ll c2, ll d, ll mn, ll mx) {

}
bool chk3(ll k, ll c, ll d) {
    ll c1 = 0, c2 = 0;
    for (int i = 1; i <= 3; i++)
        if (c != i) {
            if (c1)
    ...
    for (int i = 1; i <= n; i++)
        if (arr[d][i].c == c) {
            if (++cnt >= k) {
                if (chk2(k, c1, c2, d, arr[d][i].x[d], INF)) 
					return 1;
                if (chk2(k, c1, c2, !d, arr[d][i].x[d], INF)) 
					return 1;
	...
	for(int i=n;i;i--)
        if (arr[d][i].c == c) {
            if (++cnt >= k) {
                if (chk2(k, c1, c2, d, -INF,arr[d][i].x[d])) 
					return 1;
                if (chk2(k, c1, c2, !d, -INF,arr[d][i].x[d])) 
					return 1;
...
void solve() {
    ...
    scanf("%lld", &n);
    for (int i = 1; i <= n; i++) {
        scanf("%lld%lld%lld", &arr[0][i].x[0], &arr[0][i].x[1], &arr[0][i].c);
        arr[1][i] = arr[0][i];
    }
    sort(arr[0] + 1, arr[0] + 1 + n, cmpx);
    sort(arr[1] + 1, arr[1] + 1 + n, cmpy);
}
