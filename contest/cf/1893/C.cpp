...
ll n, m, arr[MXN];
ll sl, sr;
ll tot[MXN], l[MXN], r[MXN];
map<ll, vector<pi>> app;
ll calc(ll x) {
    auto it = app.find(x);
    if (it == app.end()) return 0;
    auto &xapp = it->second;
    ll tsl = sl, tsr = sr;
    ll res = 0;
    for (auto &i : xapp) {
        tsl -= l[i.fi];
        tsr -= r[i.fi];
        ll cl = max(l[i.fi], x - tsr);
        ll cr = min(r[i.fi], x - tsl);
        ll curva = max(cl, min(cr, tot[i.fi] - i.se));
        res += max(0ll, curva - tot[i.fi] + i.se);
...
int main() {
    ...
    while (t--) {
        ...
        for (ll i = 1; i <= n; i++) {
            ...
            sr += r[i];
            vector<pi> a(m);
            for (auto &j : a) cin >> j.fi;
            for (auto &j : a) {
                cin >> j.se;
                app[j.fi].push_back({i, j.se});
                tot[i] += j.se;
            }
        }
        if (sr - sl > MXN)
