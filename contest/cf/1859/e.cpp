    ...
    while (t--) {
        ...
        for (ll i = 0; i <= m; i++)
            for (ll j = i; j <= n; j++) {
                ...
                umx(dp[i][j], mx[j - i][0] - arr[j] + brr[j]);
                umx(dp[i][j], mx[j - i][1] - arr[j] - brr[j]);
                umx(dp[i][j], mx[j - i][2] + arr[j] + brr[j]);
                umx(dp[i][j], mx[j - i][3] + arr[j] - brr[j]);
                umx(mx[j - i][0], dp[i][j] + brr[j + 1] - arr[j + 1]);
                umx(mx[j - i][1], dp[i][j] + brr[j + 1] + arr[j + 1]);
                umx(mx[j - i][2], dp[i][j] - brr[j + 1] - arr[j + 1]);
                umx(mx[j - i][3], dp[i][j] - brr[j + 1] + arr[j + 1]);
                //if(i==0 && j==1)cout<<dp[i][j]<<mx[1][2]<<endl;
                //cout<<i<<" "<<j<<" "<<dp[i][j]<<nl;
            }
        cout << dp[m][n] << nl;
    }
