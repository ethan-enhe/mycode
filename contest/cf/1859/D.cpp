...
int main() {
    ...
    while (t--) {
        ...
        sort(arr + 1, arr + 1 + n);
        for (ll i = 1; i <= n; i++) umx(arr[i].se, arr[i - 1].se);
        for (ll i = n; i >= 1; i--) {
            ll idx = upper_bound(arr + 1, arr + 1 + n, pi{arr[i].se, INF}) - arr - 1;
            arr[i].se = arr[idx].se;
        }
        ll q;
        cin >> q;
        while (q--) {
            ll x;
            cin >> x;
            ll idx = upper_bound(arr + 1, arr + 1 + n, pi{x, INF}) - arr - 1;
            if (idx) x = max(x, arr[idx].se);
            cout << x << " ";
        }
        cout << nl;
    }
