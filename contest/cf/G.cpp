...
ll n, m, arr[MXN], s[MXN];
char str[MXN];
char dp[MXN];
#define f(i, j) dp[(i) * N + (j)]
int main() {
    ...
    while (t--) {
        ...
        for (ll i = 1; i <= n; i++) {
            cin >> str;
            for (ll i = 1; i <= m; i++) arr[i] += str[i - 1] - '0';
        }
        ll N = n * 2;
        for (ll i = 1; i <= m + 1; i++) {
        ...
        for (ll i = m + 1; i > 1; i--) {
            ...
            for (ll j = 0; j < N; j++) {
                if (f(i, j)) {
                    if (((j + cnt) & 1) == s[i - 1]) f(i - 1, (j + cnt) >> 1) = '0';
                    if (((j + alt) & 1) == s[i - 1]) f(i - 1, (j + alt) >> 1) = '1';
                }
            }
        }
        if (f(1, 0)) {
