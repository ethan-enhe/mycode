...
int main() {
    ...
    while (t--) {
        ...
        sort(all(a0), [](ll x, ll y) { return x > y; });
        sort(all(a1), [](ll x, ll y) { return x > y; });
        partial_sum(all(a0), a0.begin());
        partial_sum(all(a1), a1.begin());
        a0.insert(a0.begin(), 0);
        a1.insert(a1.begin(), 0);
        if (a0.size() > a1.size())
            ans += a0[a1.size() - 1] + a1[a1.size() - 1];
        else if (a1.size() > a0.size())
            ans += a0[a0.size() - 1] + a1[a0.size() - 1];
        else {
            ans += max(a0[a0.size() - 1] + a1[a0.size() - 2], a0[a0.size() - 2] + a1[a0.size() - 1]);
        }
        cout << ans << nl;
    }
    return 0;
