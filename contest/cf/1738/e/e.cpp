...
int main() {
    ...
    while (t--) {
        ...
        ll indl = 0, indr = n, lsum = 0, rsum = 0;
        mod ans = 1;
        arr[n + 1] = arr[0] = -1;
        while (1) {
            lsum += arr[++indl];
            while (rsum < lsum) rsum += arr[indr--];
            if (indl > indr) break;
            if (lsum == rsum) {
                ll _indl = indl, _indr = indr;
                while (arr[_indl + 1] == 0) ++_indl;
                while (arr[_indr] == 0) --_indr;
                if (_indl >= _indr) {
                    /* cerr << _indl << " " << indl << endl; */
                    ans *= qpow(mod(2), _indl - indl + (_indl != n));
                    break;
                } else {
                    ll cl = _indl - indl + (_indl != n), cr = indr - _indr + (indr != n);
                    mod tmp = 0;
                    if (cl > cr) swap(cl, cr);
                    for (ll i = 0; i <= cl; i++) tmp += c(cl, i) * c(cr, i);
                    ans *= tmp;
                }
                indl = _indl;
                indr = _indr;
