...
ll n, m, a[MXN], b[MXN];
ll sorted_a[MXN], sorted_b[MXN];

mod ans;
void mdfa(ll i) {
    ll idx = upper_bound(sorted_a + 1, sorted_a + n + 1, a[i]) - sorted_a - 1;
    ans /= min(sorted_a[idx], sorted_b[idx]);
    ans *= min(++sorted_a[idx], sorted_b[idx]);
    ++a[i];
}
void mdfb(ll i) {
    ll idx = upper_bound(sorted_b + 1, sorted_b + n + 1, b[i]) - sorted_b - 1;
    ans /= min(sorted_a[idx], sorted_b[idx]);
    ans *= min(sorted_a[idx], ++sorted_b[idx]);
    ++b[i];
}
