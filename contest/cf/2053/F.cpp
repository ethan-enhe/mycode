...
int main() {
    ...
    while (t--) {
        ...
        for (ll i = 1; i <= n; i++) {
            ...
            if (i > 1) {
                for (auto [x, y] : cnt[i]) {
                    auto it = cnt[i - 1].find(x);
                    if (it != cnt[i - 1].end()) tot += it->second * y;
                }
                tot += rem[i - 1] * rem[i];
            }
        }
        ...
        for (ll i = 1; i <= n; i++) {
            ...
            ll nmx = mx;
            for (auto [x, y] : mrg) {
                dp[x] = max(dp[x], not_deter) + y * rem[i];
                // if (i == 1 && x == 1) {
                //     cerr << x << ' ' << y << ' ' << rem[i] << ' ' << dp[x] << nl;
                // }

                umx(nmx, dp[x]);
            }
            mx = nmx;
            not_deter = max(not_deter, mx - rem[i + 1] * rem[i]);
            // cerr << i << ' ' << mx << nl;
            // for (auto [x, y] : dp) {
            //     cerr << x << ' ' << y << nl;
            // }
