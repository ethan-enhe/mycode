...
void build(ll p = 1, ll l = 1, ll r = n) {
    ...
    build(rs, mid + 1, r);
    pull(p);
        /* cerr<<l<<" "<<r<<t[p].mx<<endl; */
}
node qry(ll ql, ll qr, ll p = 1, ll l = 1, ll r = n) {
    if (ql <= l && r <= qr) {
        /* cerr<<l<<" "<<r<<t[p].mx<<endl; */
        return t[p];
    }
    if (r < ql || l > qr) return {{-INF, -1}, {INF, -1}};
    ll mid = (l + r) >> 1;
...
ll solve(ll l, ll r) {
    ...
    if(l==r)return 0;
    node res = qry(l, r);
    pi tmp = {res.mx.se, res.mn.se};
    if (tmp.fi == l || tmp.fi == r) swap(tmp.fi, tmp.se);
    if (tmp.fi == l || tmp.fi == r) return 1;
    return solve(l, tmp.fi) + solve(tmp.fi, r);
}
