...
bool f[MXN];
ll n, m, arr[MXN], dis[MXN], from[MXN];
vector<ll> g[MXN];
queue<ll> q;
bool bfs(ll x, ll dig) {
    if (g[x].size() < 4) return 0;
    q.push(x);
    for (ll i = 1; i <= n; i++) dis[i] = -1, f[i] = 0;
    dis[x] = 0;
    dis[0] = INF;
    ll lastin = 0;
    while (!q.empty()) {
        ...
        q.pop();
        for (ll nx : g[p]) {
            if (p == x && (nx >> dig) & 1) continue;
            if (nx == x && ((p >> dig) & 1) == 0) continue;
            if (dis[nx] == -1) {
                dis[nx] = dis[p] + 1;
                from[nx] = p;
                q.push(nx);
    ...
    if (lastin) {
        ...
        do {
            f[lastin] = 1;
            ans.push_back({last, lastin});
            last = lastin, lastin = from[lastin];
        } while (last != x);
        ll rem = 2;
        for (auto &i : g[x])
            if (rem && !f[i]) {
