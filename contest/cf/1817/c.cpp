...
const char nl = '\n';
const ll INF = 1e18;
const ll MXN = 2.5e6 + 5;

ll n, m;
mod arr[MXN], brr[MXN];
mod fac[MXN], ifac[MXN];
pair<mod, mod> calc(mod *x) {
    mod sum = 0;
    for (ll i = 0; i <= n; i++) sum += i;
    mod an = 0, _an = 0;
    for (ll i = 0; i <= n; i++) {
        mod mult = x[i] * ifac[i] * ifac[n - i];
        if ((n - i) & 1) mult = (mod)0 - mult;
        an += mult;
        _an += mult * ((mod)i - sum);
    }
    // cerr << an << " " << _an << nl;
...
int main() {
    ...
    cin.tie(0);
    fac[0] = 1;
    for (ll i = 1; i < MXN; i++) fac[i] = fac[i - 1] * i;
    ifac[MXN - 1] = (mod)1 / fac[MXN - 1];
    for (ll i = MXN - 1; i; i--) ifac[i - 1] = ifac[i] * i;
    cin >> n;
    ...
    pair<mod, mod> x = calc(arr), y = calc(brr);
    cout << (y.second - x.second) / x.first / n;
