...
int main() {
    ...
    cin >> t;
    while (t--) {
        for (ll i = 1; i <= n * 2; i++) sum[i] = lside[i] = 0;
        cin >> n;
        for (ll i = 1; i <= n; i++) {
            cin >> arr[i];
            if (i <= arr[i]) lside[arr[i]] = i;
            lside[arr[i] + n] = i <= arr[i] ? i + n : i;
        }
        for (ll i = 1; i <= n * 2; i++) {
            if (i > n) {
                ans[i - n] =  i - lside[i]- suf(lside[i]);
            }
            if (lside[i]) mdf(lside[i], 1);
        }
