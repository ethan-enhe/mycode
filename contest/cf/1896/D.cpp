...
int main() {
    ...
    while (t--) {
        ...
        for (ll i = 1; i <= n; i++) {
            cin >> arr[i];
            mdf(i, arr[i]);
            if (arr[i] == 1) plc1.insert(i);
        }
        auto mdfv = [&](ll x, ll y) {
            if (arr[x] == 1) plc1.erase(x);
            if (y == 1) plc1.insert(x);
            mdf(x, y - arr[x]);
            arr[x] = y;
        };
        auto chmx = [&](bool odd) {
            ll tot = pre(n);
            if ((tot & 1) == odd) return tot;
            if (plc1.empty()) return (ll)-1;
            return max(tot - pre(*plc1.begin()), pre(*plc1.rbegin() - 1));
        };
        // cout<<chmx(1);
        while (m--) {
            ll op, x, y;
