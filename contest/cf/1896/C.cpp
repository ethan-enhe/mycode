...
ll n, m, sa[MXN], arr[MXN], brr[MXN];
ll idx[MXN];
vector<ll> corrsp[MXN];
int main() {
    ...
    while (t--) {
        ...
        for (ll i = 1; i <= m; i++) {
            f &= arr[n - m + i] > brr[i];
            corrsp[arr[n - m + i]].push_back(brr[i]);
        }
        for (ll i = m + 1; i <= n; i++) {
            f &= brr[i] >= arr[i - m];
            corrsp[arr[i - m]].push_back(brr[i]);
        }
        cout << (f ? "YES" : "NO") << nl;

        if (f) {
            for (ll i = 1; i <= n; i++) cout << corrsp[sa[i]][idx[sa[i]]++] << " ";
