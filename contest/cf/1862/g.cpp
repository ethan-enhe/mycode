...
multiset<ll> delt;
multiset<ll> val;
void insd(ll x) { delt.insert(x); }
void remd(ll x) {
    auto it = delt.find(x);
    if (it != delt.end()) delt.erase(it);
}
void insv(ll x) {
...
void remv(ll x) {
    ...
    if (nxt != val.end()) remd(*nxt - *it);
    val.erase(it);
}
ll getans() { return *val.rbegin() + (n > 1 ? *delt.rbegin() : 0); }
int main() {
