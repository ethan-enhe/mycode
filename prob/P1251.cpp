...
int main() {
	...
	for(ll i=1;i<=n;i++){
		ll tmp;
		scanf("%lld",&tmp);
		fl.addedge(day(i), tnode(i), tmp, tmp, 0);
		fl.addedge(tnode(i), t, 0, INF, 0);
	}
	scanf("%lld%lld%lld%lld%lld",&_p,&_m,&_f,&_n,&_s);
	for(ll i=1;i<=n;i++){ 
		if(i==1)fl.addedge(s, day(i), 0, INF, _p);
		else fl.addedge(day(i-1), day(i), 0, INF, 0);
		if(i+_m<=n)fl.addedge(tnode(i), day(i+_m), 0, INF, _f);
		if(i+_n<=n)fl.addedge(tnode(i), day(i+_n), 0, INF, _s);
	}
	printf("%lld",fl.run(2*n+3, 2*n+4, s, t,0).second);
	return 0;
}
