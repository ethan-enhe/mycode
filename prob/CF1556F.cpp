...
const ll MXN=14;
ll n,FULL,ans;
ll arr[MXN],cost[MXN][1<<MXN],dp[1<<MXN];
il ll pro(ll s1,ll s2){
	ll res=1;
	while(s1){
		ll i=s1&(-s1);
		res=res*cost[(ll)log2(i)][s2]%P;
		s1^=i;
	}
	return res;
}
...
il void solve(){
	...
	for(ll i=0;i<n;i++){
		for(ll j=0;j<n;j++)
			cost[i][1<<j]=arr[i]*inv(arr[i]+arr[j])%P;
		cost[i][0]=1;
		for(ll j=1;j!=FULL;j++){
			ll lbt=j&(-j);
			cost[i][j]=cost[i][j^lbt]*cost[i][lbt]%P;
		}
	}
	for(ll i=1;i!=FULL;i++){
		dp[i]=1;
		for(ll j=(i-1)&i;j;j=(j-1)&i)
			madd(dp[i],-dp[j]*pro(j,i^j));
		madd(ans,__builtin_popcount(i)*dp[i]%P*pro(i,(FULL-1)^i));
	}
	printf("%lld",(ans+P)%P);
