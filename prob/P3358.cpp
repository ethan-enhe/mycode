...
int main() {
	...
	for (ll i = 1; i <= n; i++) {
		ll y1, y2;
		scanf("%lld%lld%lld%lld", l + i, &y1, r + i, &y2);
		if (r[i] < l[i])
			swap(l[i], r[i]), swap(y1, y2);
		len[i] = sqrt((r[i] - l[i]) * (r[i] - l[i]) + (y1 - y2) * (y1 - y2));
		pool[i] = l[i], pool[i + n] = r[i];
	}
	sz = n << 1;
	sort(pool + 1, pool + 1 + sz);
	// sz = unique(pool + 1, pool + 1 + sz) - pool - 1;
	for (ll i = 1; i <= n; i++) {
		if(l[i]==r[i]){
			l[i] = lower_bound(pool + 1, pool + 1 + sz, l[i]) - pool + 1;
			r[i] = lower_bound(pool + 1, pool + 1 + sz, r[i]) - pool + 2;
		}
		else{
			l[i] = upper_bound(pool + 1, pool + 1 + sz, l[i]) - pool ;
			r[i] = lower_bound(pool + 1, pool + 1 + sz, r[i]) - pool + 1;
		}
		f.addedge(l[i], r[i], 1, -len[i]);
	}
	++sz;
	for (ll i = 1; i <= sz; i++)
		f.addedge(i, i + 1, m, 0);
	auto res = f.run(1, sz+1);
