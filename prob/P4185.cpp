...
struct ele{
	ll x,y,z;
	inline bool operator < (const ele &b)const{return z>b.z;}
}edge[MXN],que[MXN];
...
il void solve(){
	...
	for(int i=1;i<n;i++)cin>>edge[i].x>>edge[i].y>>edge[i].z;
	for(int i=1;i<=m;i++){
		cin>>que[i].z>>que[i].x;
		que[i].y=i;
	}
	...
	ll ind=1;
	for(int i=1;i<=m;i++){
		while(ind<=n-1 && edge[ind].z>=que[i].z){
			merge(edge[ind].x,edge[ind].y);
			ind++;
		}
		ans[que[i].y]=sz[find(que[i].x)];
	}
	for(int i=1;i<=m;i++)cout<<ans[i]-1<<nl;
}
