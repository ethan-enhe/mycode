...
void add(ll x, ll y, ll z, ll v, bool f, bool g) {
    if (f && g)
        va[z] += v;
    else if (f ^ g) {
        if (f)
...
void prt() {
    cout << tot << endl;
    for (int i = 1; i <= nodec; i++) cout << i << ": " << va[i] << endl;
    cout << "---" << endl;
}
ll cal() {
    for (int i = 1; i <= nodec; i++) {
        if (va[i] > 0) {
            tot += va[i];
            f.adduedge(nodec + 1, i, va[i]);
        } else
            f.adduedge(i, nodec + 2, -va[i]);
    }
    return f.run(nodec + 1, nodec + 2).first - tot;
}
