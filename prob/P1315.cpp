...
il void renum(ll *v, ll sz) {
    ...
    for (ll i = 1; i <= sz; i++) v[i] = lower_bound(pool + 1, pool + 1 + sz, v[i]) - pool;
}
//}}}
//}}}
//{{{ Algo
...
il void solve() {
    ...
    for (ll i = n; i; i--) {
        act[i] -= mnt[i];
        mni[0][i] = i;
        for (ll j = 1; i + (1 << j) <= n + 1; j++) mni[j][i] = mini(mni[j - 1][i], mni[j - 1][i + (1 << (j - 1))]);
    }
    ll last = 1;
    for (ll i = 2; i <= n; i++)
        if (act[i] <= 0) {
