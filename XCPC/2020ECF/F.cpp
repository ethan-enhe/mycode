...
int main() {
    ...
    for (ll i = 1; i <= n; i++) {
        ll x, y;
        cin >> x >> y;
        xc[x].push_back({y, i, 0});
        yc[y].push_back({x, i, 0});
    }
    for (ll i = 1; i <= m; i++) {
        ll x, y;
        cin >> x >> y;
        xc[x].push_back({y, i, 1});
        yc[y].push_back({x, i, 1});
    }
    for (auto &i : xc) {
        sort(i.second.begin(), i.second.end());
        for (ll j = 1; j < i.second.size(); j++) {
            if (i.second[j].tp != i.second[j - 1].tp) {
                att[i.second[j].tp][i.second[j].id] = 1;
                att[i.second[j - 1].tp][i.second[j - 1].id] = 1;
            }
        }
    }
    for (auto &i : yc) {
    ...
    for (ll i = 1; i <= n; i++) cout << att[0][i];
    cout << nl;
    for (ll i = 1; i <= m; i++) cout << att[1][i];
    return 0;
}
