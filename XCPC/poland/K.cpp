...
int main() {
    ...
    while (t--) {
        ...
        for (ll i = 1; i <= n; i++)
            for (ll j = i; j <= n; j++) cnt[i][j] = 0;
        for (ll i = 1; i <= n; i++)
            for (ll j = i + 1; j <= n; j++)
                if (arr[i] > arr[j]) ++cnt[i][j];
        for (ll i = n; i; i--)
            for (ll j = i + 1; j <= n; j++) cnt[i][j] += cnt[i + 1][j] + cnt[i][j - 1] - cnt[i + 1][j - 1];
        for (ll i = 1; i <= n; i++) dp[i] = cnt[1][i];
        // for (ll i = 1; i <= n; i++)
        //     for (ll j = i + 1; j <= n; j++)
        //         for (ll k = j + 1; k <= n; k++)
        //             for (ll l = k + 1; l <= n; l++) assert(cnt[i][l] + cnt[j][k] >= cnt[i][k] + cnt[j][k]);
    }
    return 0;
}
