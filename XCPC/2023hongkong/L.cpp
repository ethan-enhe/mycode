...
int main() {
    ...
    while (t--) {
        ...
        magic.clear();
        greater_pos.clear();
        greater_pos.insert(0);
        greater_pos.insert(n + 1);
        for (ll i = 1; i <= n; i++) {
            brr[i] = 0;
            sum[i] = 0;
        }
        ...
        if (f)
            for (ll i = n; i; i--) {
                if (brr[i]) {
                    greater_pos.insert(arr[i]);
                    continue;
                }
                auto nxt = greater_pos.lower_bound(arr[i]);
                auto p = prev(nxt);
                ll mxc = pre(*nxt - 1) - pre(*p);
                auto it = magic.upper_bound(mxc);
                if (it == magic.begin()) {
                    f = 0;
                    break;
                }
